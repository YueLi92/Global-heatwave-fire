%% Updated Extended Data Fig. 9 using duration effects
clear,clc;
...
load('D:\Study\fires\Extreme_fires_relationship\MODISv61_newanalysis\si_fig_9_firepredict\fut_sta1.mat','dylenall','httavgall','httavgall_std','httavgalldur','httavgalldur_std');
...
for kk = 1 : 8
    ...
    load(['D:\Study\fires\Extreme_fires_relationship\MODISv61_newanalysis\2026.04.02.cumulative_VPD\fire_dur_region_',num2str(ri),'.mat'])    
    if(ri == 42)
        zzz = nan(10,19);
        zzz(1:9,:) = fnmat;
        fnmat = zzz;
    ...
    fn_sensdur = reshape(fnmean(4:10),1,7);
    szmean = nansum(szmat,2)./nansum(hdmat,2);
    ba_sensdur = reshape(szmean(4:10),1,7);  
    httobsdur = reshape(nanmean(hdmat(4:10,:),2),1,7);
    fn_sensdur(isnan(fn_sensdur)) = 0;
    ba_sensdur(isnan(ba_sensdur)) = 0;
    ...
    httobsdur(7) = mean(htwdobs)-sum(httobsdur)+httobsdur(7);
%     httobsdur  = round(httobsdur*10)/10;
    ...
    htt2 = nan(70,100,7);
    for ii = 1 : 100 % need to update to 100
        if(ii <=50)
            load(['D:\Study\fires\Extreme_fires_relationship\2021.11.18.cesm2_les\draw_script\d1_htwv_ratio_new_reg51new21_7d\htwv_ratio.',ensnb{ii},'.region_',num2str(ri),'.mat']);
        else
            load(['D:\Study\fires\Extreme_fires_relationship\2021.11.18.cesm2_les\draw_script\d1_htwv_ratio_new_reg51new21_7d\htwv_ratio.',ensnb2{ii-50},'.region_',num2str(ri),'.mat']);
        end
        ...
        for i = 1 : 70
            ...
            for wi = 1 : 7
                if(flag ==1)
                    htt2(i,ii,wi) = sum(tmp(wi,bgday1:edday1)) + sum(tmp(wi,bgday2:edday2));
                else
                    htt2(i,ii,wi) = sum(tmp(wi,bgday:edday));
                end
            end
    ...
    htt_residu = htt - sum(htt2,3);
%     htt_residu(htt_residu < 0) = 0;
    htt2(:,:,7) = htt2(:,:,7) + htt_residu;
%     httavgalldur(1,kk,:) = mean(mean(htt2(2003-1979:2021-1979,:,:),1),2);
%     httavgalldur_std(1,kk,:) = std(mean(htt2(2003-1979:2021-1979,:,:),1),0,2);
%     httavgalldur(2,kk,:) = mean(mean(htt2(2026-1979:2040-1979,:,:),1),2);
%     httavgalldur_std(2,kk,:) = std(mean(htt2(2026-1979:2040-1979,:,:),1),0,2);
...
%         p1(ii) = plot([1981:1:2050]',htt(:,ii)-mean(htt(2003-1980:2018-1980,ii))+mean(htwdobs));
    ...
    httavg = mean(htt,2);
    httavg2 = reshape(mean(htt2,2),70,7);
%     httavg2 = round(httavg2*10)/10;
    httstd = std(htt,0,2);
    x = [1981:1:2040]';
    httavgall(:,kk) = httavg;
    httavg = httavg - mean(httavg(2003-1980:2021-1980)) + mean(htwdobs);
    httavg2 = httavg2 - repmat(mean(httavg2(2003-1980:2021-1980,:),1),70,1) + repmat(httobsdur,70,1);
    httavg2(isnan(httavg2)) = 0;
...
%     l11 = line([1980 2050],[mean(htwdobs) mean(htwdobs)],'Color',[0.8 0.8 0.8],'LineWidth',1.2);
    ...
    text(1990,35,['Ensemble mean: +',num2str(   round((mean(httavg(end-24:end-10))-mean(httavg(2003-1980:2021-1980)))*10)/10  )])
%     hold on,
...
%         p1(ii) = plot([1981:1:2040]',(zz*dataall(4,2,1,ri)+(dylen-zz)*dataall(4,1,1,ri)));
        cumd = zeros(60,1);
        for duri = 1 : 7
            cumd = cumd + reshape(zz2(:,1,duri)*fn_sensdur(duri),60,1);
        end
        p1(ii) = plot([1981:1:2040]',(cumd+(dylen-zz)*dataall(4,1,1,ri)));
        p1(ii).Color = [0.8 0.8 0.8 0.3];
        p1(ii).LineWidth = 1.2;
        hold on,
    end
    ...
    cumd = zeros(70,1);
    for duri = 1 : 7
        cumd = cumd + reshape(httavg2(:,duri)*fn_sensdur(duri),70,1);
    end
    firenbprdct = (cumd+(dylen-httavg)*dataall(4,1,1,ri));
    if(ri ==1)
    mean(httavg(end-24:end-10))
    dataall(4,2,1,ri)
    end
    ...
    yllm = get(gca,'YLim');
    text(1990,(yllm(2)-yllm(1))*7/8+yllm(1),['Ensemble mean: +',num2str(   round((mean(firenbprdct(end-24:end-10))-mean(firenbprdct(2003-1980:2021-1980)))/mean(firenbprdct(2003-1980:2021-1980))*100)/1  ),'%'])
    dataarray2(kk,1) = mean(firenbprdct(end-24:end-10))-mean(firenbprdct(2003-1980:2021-1980));
%     text(2000,(yllm(2)-yllm(1))*7/8+yllm(1),['+',num2str(   round((mean(firenbprdct(end-25:end-10))-mean(firenbprdct(2003-1980:2021-1980))))/1  ),''])
...
%         p1(ii) = plot([1981:1:2040]',(zz*dataall(5,2,1,ri)+(dylen-zz)*dataall(5,1,1,ri)));
        cumd = zeros(60,1);
        for duri = 1 : 7
            cumd = cumd + reshape(zz2(:,1,duri)*ba_sensdur(duri),60,1);
        end
        p1(ii) = plot([1981:1:2040]',(cumd+(dylen-zz)*dataall(5,1,1,ri)));
        p1(ii).Color = [0.8 0.8 0.8 0.3];
        p1(ii).LineWidth = 1.2;
        hold on,
    end
...
%     fireszprdct = (httavg*dataall(5,2,1,ri)+(dylen-httavg)*dataall(5,1,1,ri));
    cumd = zeros(70,1);
    for duri = 1 : 7
        cumd = cumd + reshape(httavg2(:,duri)*ba_sensdur(duri),70,1);
    end
    fireszprdct = (cumd+(dylen-httavg)*dataall(5,1,1,ri));
    ...
    yllm = get(gca,'YLim');
    text(1990,(yllm(2)-yllm(1))*7/8+yllm(1),['Ensemble mean: +',num2str(   round((mean(fireszprdct(end-24:end-10))-mean(fireszprdct(2003-1980:2021-1980)))/mean(fireszprdct(2003-1980:2021-1980))*100)/1  ),'%'])
    dataarray2(kk,2) =(mean(fireszprdct(end-24:end-10))-mean(fireszprdct(2003-1980:2021-1980)));
%     text(2000,(yllm(2)-yllm(1))*7/8+yllm(1),['+',num2str(   round((mean(fireszprdct(end-25:end-10))-mean(fireszprdct(2003-1980:2021-1980))))/1  ),''])
...
set(gcf,'position',[304          -1000        1012        4013])
